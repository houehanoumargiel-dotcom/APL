...
module.exports = async (req, res) => {
  // Autoriser CORS pour les requêtes du frontend (optionnel en production)
  ...
  try {
    // 1. Appel à Mistral
    const mistralContent = await callMistral(subject);
    const ebookData = JSON.parse(mistralContent);

    // 2. Génération des images (si clé OpenAI présente)
    ...
    return res.status(200).json(ebookData);
  } catch (error) {
